...
parser.add_argument("--crop_size",default=144,type=int) #72  144 148
...
def GAN_train(args):
    ...
    genOutput = model.generator_stack(genInput,args=args,name='generator')
    ...
    discr_outlabel1,discr_outlabel2,discr_outlabel3 = model.discriminator_mult(genLabel,args=args,name='discriminator')
    discr_outGenout1,discr_outGenout2,discr_outGenout3 = model.discriminator_mult(genOutput[1],args=args,reuse=True,name='discriminator')
    gen_loss = model.gen_loss_mult(genOutput,genLabel,discr_outGenout1,discr_outGenout2,discr_outGenout3,args.EPS,args.perceptual_mode)
    dis_loss = model.discr_loss_mult(discr_outGenout1,discr_outlabel1,discr_outGenout2,discr_outlabel2,discr_outGenout3,discr_outlabel3)


    PSNR = compute_psnr(genOutput[1],genLabel,convert=True)
...
def adtest(args):
    ...
    y = model.generator_stack(x, args=args,name='generator')

    y_up = tf.placeholder(tf.float32, shape=[1, 148, 148, 3])
    y_down = tf.placeholder(tf.float32, shape=[1, 148, 148, 3])
    y_down_label = tf.nn.avg_pool(y_,[1,2,2,1],[1,2,2,1],padding='SAME')

    loss = tf.reduce_mean(tf.square(y_up - y_))
    PSNR = compute_psnr(y_up, y_)
    PSNR_scale_down = compute_psnr(y_down, y_down_label)
    ...
    for i in range(len(blur_file)):
        ...
        output_up = output[1]
        loss_test = sess.run(loss,feed_dict={y_up:output_up,y_:img_label})
        PSNR_test_down = sess.run(PSNR_scale_down,feed_dict={y_down:output_down,y_:img_label})
        PSNR_test = sess.run(PSNR,feed_dict={y_up:output_up,y_:img_label})

        output_down = (output_down+1)*(255/2)
        output_down = np.squeeze(output_down).astype(np.uint8)
        output_up = (output_up+1)*(255/2)
        output_up = np.squeeze(output_up).astype(np.uint8)

        cv.imwrite('./output/output_face/deblur_down_0'+str(i+1)+'.png',output_down,[int(cv.IMWRITE_PNG_COMPRESSION), 0])
        cv.imwrite('./output/output_face/deblur_up_0'+str(i+1)+'.png',output_up,[int(cv.IMWRITE_PNG_COMPRESSION), 0])
        print('loss_test:[%.8f],PSNR_test_down:[%.8f],PSNR_test_up:[%.8f]' % (loss_test,PSNR_test_down,PSNR_test))
